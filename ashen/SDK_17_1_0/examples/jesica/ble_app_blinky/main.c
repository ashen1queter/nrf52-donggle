...
#define NEXT_CONN_PARAMS_UPDATE_DELAY                                                                                  \
  APP_TIMER_TICKS(5000)  /**< Time between each call to sd_ble_gap_conn_param_update after the first call (5 seconds). \
                          */
#define MAX_CONN_PARAMS_UPDATE_COUNT 3 /**< Number of attempts before giving up the connection parameter negotiation. \
                                        */

#define BUTTON_DETECTION_DELAY                                                                                        \
  APP_TIMER_TICKS(50) /**< Delay from a GPIOTE event until a button is reported as pushed (in number of timer ticks). \
                       */
...
static char ascii_buf[4];

static bool long_press_in_progress = false;
...
static void timeout_handler(void *p_context) {
  ret_code_t err_code;
  ++button_press_count;
  snprintf(ascii_buf, sizeof(ascii_buf), "%u", button_press_count);
  err_code               = ble_lbs_on_button_change(m_conn_handle, &m_lbs, ascii_buf, strlen(ascii_buf));
  long_press_in_progress = true;
...
static void services_init(void) {
  ...
  // init.button_char_handles.value_handle = button_press_count;
...
static void button_event_handler(uint8_t pin_no, uint8_t button_action) {
  ...
  case LEDBUTTON_BUTTON:
    ...
    if (button_action) {
      ...
      if (!long_press_in_progress) {
        ++button_press_count;
        snprintf(ascii_buf, sizeof(ascii_buf), "%u", button_press_count);

        ret_code_t err_code = ble_lbs_on_button_change(m_conn_handle, &m_lbs, ascii_buf, strlen(ascii_buf));
